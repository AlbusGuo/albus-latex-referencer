import { getLinkpath, Keymap, editorInfoField, editorLivePreviewField, finishRenderMath, TFile, App } from "obsidian";
...
/** Add supercharged-links attributes to an element */
function addSuperChargedAttributes(app: App, span: HTMLElement, file: TFile): void {
    if (file && app.plugins.enabledPlugins.has("supercharged-links-obsidian")) {
        const cache = app.metadataCache.getFileCache(file);
        const data = (app.plugins.plugins as any)["supercharged-links-obsidian"]?.settings;
        
        // Add tags
        let tagArr = cache?.tags;
        let tags: string = "";
        if (tagArr) {
        ...
        span.setAttribute("data-link-path", file.path);
        span.setAttribute("data-link-tags", tags);
        for (let i = 0; i < attributes.length; i++)
            span.setAttribute("data-link-" + attributes[i][0], attributes[i][1]);
    }
}
...
/** Given a plugin instance, create a CodeMirror6 view plugin that renders MathLink in links. */
export const createEditorExtensions = (plugin: LatexReferencer): Extension => {
    const { app } = plugin;
    const forceUpdateEffect = plugin.mathLinksManager?.forceUpdateEffect;
    if (!forceUpdateEffect) throw new Error("MathLinksManager not initialized");
    ...
    class MathWidget extends WidgetType {
         ...
         * @param sourcePath The path of the file associated with the current editor view.
         */
        ...
        toDOM() {
            ...
            // Add supercharged-links support
            if (targetFile && app.plugins.enabledPlugins.has("supercharged-links-obsidian")) {
                addSuperChargedAttributes(app, mathLinkEl, targetFile);
            }
    ...
    const viewPlugin = ViewPlugin.fromClass(
        class implements PluginValue {
            ...
            tryBuildingDecorations(view: EditorView): void {
                ...
                if (isSourceMode && plugin.mathLinksManager && !plugin.mathLinksManager.enableInSourceMode()) {
